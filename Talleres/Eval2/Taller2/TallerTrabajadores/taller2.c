...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        printf("Error: Numero de parametros invalido.\nDebe iniciar asi: ./taller.out <archivo.csv> <salida.txt>\n");
		exit(0);
    }
    ...
    if(exitValue == 0) {
        printf("se cerro el archivo employees\n");
    }
    exitValue = fclose(salida);
    if(exitValue == 0) {
        printf("se cerro el archivo salida\n");
    }
...
void registerEmployees(employee *emp, FILE *f) {
    ...
    for (i = 0; i < numEmployees; i++) {
        ...
        while(token!=NULL) {
            ...
            token = strtok(NULL, ";");
            emp->valorExtras = EXTRA*(emp->extraHours);
        }
        emp++;
    }
    printf("Los usuarios fueron registrados exitosamente\n");
}
...
float getTotal(employee *emp) {
    ...
    float total = 0;
    for (i = 0; i < numEmployees; i++) {
        total += emp->salary+(emp->extraHours*EXTRA);
        emp++;
    }
...
void showEmployees(employee *emp, FILE *f) {
    ...
    fprintf(f, "|%-12s|%-12s|%-12s|%-12s|\n", "Nombre", "Salario", "Horas extra", "Valor Extras");
    for (i = 0; i < numEmployees; i++) {
        fprintf(f, "|%-12s|%-12.2f|%-12d|%-12.2f|\n", emp->name, emp->salary, emp->extraHours, emp->valorExtras);
        emp++;
    }
